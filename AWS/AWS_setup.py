...
def setup_jumpbox(basename='jumpbox', subnet_zone='us-west-2c'): # The jumpbox is much more configurable than the cloud shell.
    ...
    # This "random" error seems to not be random and can't be waited out?
    #f_try = lambda: AWS_core.create('machine', basename+'_VM',**vm_params)
    #f_catch = lambda e: 'is not supported in your requested Availability Zone' in repr(e)
    #msg = 'Random not supported AZ errors, retrying'
    #inst_id = AWS_core.loop_try(f_try, f_catch, msg, delay=4)
    ...
    f_try = lambda: ec2c.associate_address(AllocationId=addr['AllocationId'],InstanceId=inst_id)
